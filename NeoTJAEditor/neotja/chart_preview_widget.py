...
class ChartPreviewWidget(QWidget):
    ...
    def __init__(self, parent=None, course_select_cb=None, toggle_play_cb=None,
                 ...
                 hit_sound_engine=None, branch_select_cb=None):
    ...
    def _on_tick(self):
        ...
        if self._hit_sound_engine is not None:
    ...
    def keyPressEvent(self, event):
        ...
        if key == Qt.Key_Space:
            ...
            return
        if key == Qt.Key_Q:
            if self._seek_cursor_cb:
                self._seek_cursor_cb()
            return
    ...
    def seek_relative_measure(self, direction: int):
        if not self._bar_times or not self._seek_seconds_cb:
            return
        now_chart = self._current_chart_time()
        idx = max(0, bisect.bisect_right(self._bar_times, now_chart) - 1)
        idx = max(0, min(idx + direction, len(self._bar_times) - 1))
        # OFFSET convention: audio_time = chart_time - OFFSET.
        self._seek_seconds_cb(max(0.0, self._bar_times[idx] - self._offset))
    ...
    def set_playback(self, position_seconds: float, playing: bool):
        ...
        if self._playing and playing:
            predicted = self._pos_sec + (now_wall - self._pos_wall)
    ...
    def _current_audio_time(self) -> float:
        return self._pos_sec + (_time.monotonic() - self._pos_wall) if self._playing else self._pos_sec
