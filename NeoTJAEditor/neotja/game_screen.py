...
# 左パネル(スコア/コンボ/太鼓/銘板)。実機と輪郭を重ねて合わせた位置。
# 以前は 188 で、パネル一式が実機より 4px 下にあった(レーンは合っていたので
# パネルだけがずれていた)。**スコアの数字はここでは動かさない** — あちらは
# 絶対座標で既に実機と一致していて、パネルが上がることで実機と同じ
# 「パネルの縁から 15px 下」に収まる。
PANEL_X, PANEL_Y = 0, 184
...
# レーンの位置。**実機と重ねて合わせた。** 以前は 196 で、判定円(音符が通る
# 線)が実機より 5.5px 下にあった。
#
#   実機の判定円の中心 y = 256.5 / 外径 106
#   196 のとき 262.0 (-5.5 ずれ) / 191 のとき 256.0 (-0.5)
#
# レーンの箱の上端だけは 3px ほど上へ行き過ぎるが、枠の絵はスキンごとに
# 違うので、音符が通る線を優先した。レーン下端と下背景の隙間も、実機 42 に
# 対して 38 -> 43 と近づく。
#
# 枠・判定文字「良」・連打の扇・風船の口は、どれもここからの相対で位置を
# 出しているので、この1行で一緒に動く。
LANE_X, LANE_Y = 333, 191
...
SCORE_RIGHT, SCORE_Y = 174, 193      # スコアは右詰め
SCORE_SCALE = 1.02
...
COURSE_SYM_POS = (26, 233)           # コース記号(おに 等)
DRUM_POS = (208, 205)                # 太鼓 120x133
...
NAMEPLATE_POS = (-25, 287)           # 1P 銘板(素材の左余白ぶん外へ出す)
...
#: 合計スコアの字送り。**加算文字と同じ字間**にするための値。
#: 実際の送り幅は「1枠の幅 x 倍率 x この割合」なので、合計スコアのほうが
#: 字が大きい(1.02 対 0.902)ぶん、割合を倍率の比で割ってやると送り幅が
#: そろう。字の大きさは変えずに、間隔だけを合わせたいときの出し方。
SCORE_TOTAL_ADVANCE = SCORE_ADVANCE * SCORE_GAIN_SCALE / SCORE_SCALE
...
class GameScreenWidget(QWidget):
    ...
    def _draw_left_panel(self, p, combo, score, recent, now):
        ...
        self._draw_digits(p, self._skin.get("score_digits"), score,
                          cols=10, rows=3, row=0, advance=SCORE_TOTAL_ADVANCE,
