...
def default_settings() -> dict:
    return {
        ...
        # 譜面プレビューの最大再描画fps(CPU負荷の上限)。20〜1000。
        # 既定は 1000 — GPU 描画で実際に出るのが 366fps 程度なので、この値なら
        # 頭打ちにかからない。数字で持たせてあるのは、非力な機械で下げられる
        # ようにするため(60 など)。
        # 0 は「無制限」として扱う(12.1.4 の既定。古い設定ファイル向け)。
        # (120 以下のときは、パネルのリフレッシュレートの2倍と比べて小さい
        #  ほうを採る。ソフト描画のちらつき/カクつき対策。)
        "preview_max_fps": 1000,
        ...
        "gpu_render": True,
        # GPU 描画のとき、モニタの走査に合わせて待つか。既定 False。
        ...
        # 伸びた。入力と音のタイミングに響きうるので既定は切ってある。
        # 絵が横に裂ける現象(ティアリング)が気になるときだけ入れること。
